...
def HCD_loss_(beta, downsampled):
    def HCD_loss(y_pred, y_true, sdf_map):
        ...
        sdf_score = sample_sdf(y_pred, sdf_map)
...
def trainer(train_dataset, val_dataset, model, config):
    ...
    train_kld_loss_avg = []
    train_seg_loss_avg = []
    train_pts_all_loss_avg = []
    train_pts_out_loss_avg = []
    val_loss_avg = []
    ...
    for epoch in range(config['epochs']):
        ...
        train_pts_all_loss_avg.append(0)
        train_pts_out_loss_avg.append(0)
        train_kld_loss_avg.append(0)
        train_seg_loss_avg.append(0)
        num_batches = 0
        
        for sample_batched in train_loader:
            image, sdf, mask, target = sample_batched['image'].to(device), sample_batched['sdf'].to(device), sample_batched['mask'].to(device), sample_batched['landmarks'].to(device)
            out = model(image)
            target_down = pool(target, model.downsample_matrices[0])
            
            optimizer.zero_grad()

            pos7, pos6, pos5, pos4, pos3, pos2, pos1, seg = out
            ...
            point_loss = pos1loss + pos2loss + pos3loss + pos4loss + pos5loss + pos6loss + outloss
            dense_loss = dice_loss(seg, mask) + ce_loss(seg, mask)

            loss = point_loss + (config['seg_weight'] * dense_loss)

            train_pts_all_loss_avg[-1] += point_loss.item()
            train_pts_out_loss_avg[-1] += outloss.item()

            train_seg_loss_avg[-1] += (config['seg_weight'] * dense_loss.item())
        ...
        train_pts_all_loss_avg[-1] /= num_batches
        train_pts_out_loss_avg[-1] /= num_batches
        train_seg_loss_avg[-1] /= num_batches
        train_kld_loss_avg[-1] /= num_batches

        print('PTS:SEG:KLD ratio {:.5f}:{:.5f}:{:.5f} '.format(train_pts_all_loss_avg[-1],train_seg_loss_avg[-1], train_kld_loss_avg[-1]))

        print('Epoch [%d / %d] train average reconstruction error: %f' % (epoch+1, config['epochs'], train_pts_out_loss_avg[-1]*512*512))
        ...
        with torch.no_grad():
            for sample_batched in val_loader:
                ...
                point_loss = HCD_loss(pts, target, sdf)
                dense_loss = dice_loss(seg, mask) + ce_loss(seg, mask)
                loss_rec = point_loss + (config['seg_weight'] * dense_loss)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--name", type=str, default=datetime.datetime.now().strftime('%m_%d_%H_%M_%S')+'_MULTI_TASK')
    ...
    parser.add_argument("--seg_weight", default=0.05, type=float)
    ...
    print('Seg Loss Weights', config['seg_weight'])
    ...
    print('Model: Multi-Task Network')
    model = Multi_Task(config, D_t, U_t, A_t)
