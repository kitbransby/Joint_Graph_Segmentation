    ...
    for epoch in range(config['epochs']):
        ...
        for sample_batched in train_loader:
            ...
            if type(out) is not tuple:
                ...
                pre1loss = HCD_loss_downsampled(pre1, target_down, sdf)
                pre2loss = HCD_loss(pre2, target, sdf)
                outloss = HCD_loss(out, target, sdf)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--name", type=str, default=datetime.datetime.now().strftime('%m_%d_%H_%M_%S')+'_HCD')
