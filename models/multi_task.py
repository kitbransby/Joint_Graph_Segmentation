...
class EncoderConv(nn.Module):
    def __init__(self, config, latents = 64, hw = 32):
...
class Multi_Task(nn.Module):
    def __init__(self, config, downsample_matrices, upsample_matrices, adjacency_matrices):
        ...
        self.encoder = EncoderConv(config=config, latents = self.z, hw = hw)
        ...
        self.dconv_up5 = residualBlock(outsize1 + outsize2, outsize2)
        self.dconv_up4 = residualBlock(outsize2 + outsize3, outsize3)
        self.dconv_up3 = residualBlock(outsize3 + outsize4, outsize4)
        self.dconv_up2 = residualBlock(outsize4 + outsize5, outsize5)
        self.dconv_up1 = residualBlock(outsize5 + outsize6, outsize6)
        self.conv_last = nn.Conv2d(outsize6, config['n_classes'], 1)
    ...
    def forward(self, x):
        ...
        return pos7, pos6, pos5, pos4, pos3, pos2, pos1, conv1up