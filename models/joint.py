...
class EncoderConv(nn.Module):
    def __init__(self, config, latents = 64, hw = 32, unet=None):
        ...
        self.dconv_down2 = residualBlock(self.size[0]+8+16, self.size[1])
        self.dconv_down3 = residualBlock(self.size[1]+32, self.size[2])
        self.dconv_down4 = residualBlock(self.size[2]+64, self.size[3])
        self.dconv_down5 = residualBlock(self.size[3]+128, self.size[4])
        self.dconv_down6 = residualBlock(self.size[4]+128, self.size[4])
    ...
    def forward(self, x):
        #print([u.shape for u in self.unet_decoder_feats(x)])
        unet6, unet5, unet4, unet3, unet2, unet1, unet0 = self.unet_decoder_feats(x)


        x = torch.cat([x, unet0], dim=1)
        #print(x.shape)

        conv1 = self.dconv_down1(x)
        x = torch.cat([conv1, unet1], dim=1)
        x = self.maxpool(x)
        #print('E1: ', x.shape)
        #print(x.shape, unet2.shape)
