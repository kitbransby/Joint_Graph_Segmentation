...
class Hybrid_Rasterize(nn.Module):
    ...
    def rasterize(self, x, resolution, scale=1, offset=-0.50):
        ...
        HEART_raster = self.RASTERIZER(HEART * float(resolution) + offset, resolution, resolution, 0.1)
        ...
        BACKGROUND -= (LUNG_raster + HEART_raster)
        BACKGROUND = torch.clip(BACKGROUND, 0, 1)

        raster = torch.stack([BACKGROUND, LUNG_raster, HEART_raster], dim=1)
